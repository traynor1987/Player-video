...
@Composable private fun SettingsScreen(container: AppContainer) {
    ...
    LazyColumn(Modifier.fillMaxSize().padding(24.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        items(sources, key = { it.id }) { source -> ListItem({ Text(source.name) }, supportingContent = { Text(if (refreshState.sourceId == source.id && refreshState.message != null) refreshState.message else "${source.type.name.replace('_',' ')} • ${source.lastRefreshedAt?.let { "Last refreshed ${java.text.DateFormat.getDateTimeInstance().format(it)}" } ?: "Not refreshed"}") }, leadingContent = { Icon(Icons.Default.Storage, null) }, trailingContent = { IconButton({ refreshModel.refresh(source.id) }, enabled = refreshState.sourceId != source.id || refreshState.failed) { Icon(Icons.Default.Refresh, "Refresh source") } }, modifier = Modifier.clip(RoundedCornerShape(14.dp)).background(MaterialTheme.colorScheme.surfaceVariant)) }
