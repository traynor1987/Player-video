...
@Composable fun VideoPlayer(channelId: Long, container: AppContainer, enterPip: () -> Unit, close: () -> Unit) {
    ...
    val player = remember { ExoPlayer.Builder(context).setLoadControl(DefaultLoadControl.Builder().setBufferDurationsMs(1_500, 15_000, 500, 1_000).build()).build().apply { playWhenReady = true } }
    ...
    DisposableEffect(player) {
        val listener = object : Player.Listener {
            ...
            override fun onPlayerError(playerError: PlaybackException) { buffering = false; error = when (playerError.errorCode) { PlaybackException.ERROR_CODE_IO_NETWORK_CONNECTION_FAILED, PlaybackException.ERROR_CODE_IO_NETWORK_CONNECTION_TIMEOUT -> "Network connection interrupted"; else -> "This stream could not be played. The source may use a format this device does not support." } }
        }
    ...
    Box(Modifier.fillMaxSize().background(Color.Black).focusable().onPreviewKeyEvent { event ->
        ...
        if (buffering && error == null) Row(Modifier.align(Alignment.Center).background(Color.Black.copy(alpha = .6f), MaterialTheme.shapes.large).padding(18.dp), verticalAlignment = Alignment.CenterVertically) { CircularProgressIndicator(Modifier.size(22.dp), strokeWidth = 2.dp, color = Color.White); Spacer(Modifier.width(12.dp)); Text("Buffering stream…", color = Color.White) }
